...
同態加密是一種特別的加密方法，它允許人們對密文進行特定形式的代數運算得到仍然是加密的結果，將得到的密文解密所得到的結果與對明文進行同樣的運算結果是一樣的。
而沒有密鑰的用戶，不可能從加密結果中得到有關原始數據的任何信息。只有擁有密鑰的用戶才能夠正確解密，得到原始的內容。
過程中用戶是不能對加密結果做任何操作的，只能進行存儲、傳輸。對加密結果做任何操作，都將會導致錯誤的解密，甚至解密失敗。
同態加密保護的是數據本身，即可以讓第三方看不到處理過程的前提下，對數據進行處理。

![](https://i.imgur.com/fpCxDXF.jpg)

上圖為Alice通過Cloud，以Homomorphic Encryption處理數據的處理過程：
* Alice對數據進行加密。並把加密後的數據發送給Cloud；Alice向Cloud提交數據的處理方法(函數f)；
* Cloud利用用戶給定的數據處理方法f對數據密文進行處理，並且將處理後一樣是加密狀態的結果發送給Alice；
* Alice再對數據進行解密，即可得到結果。

這項技術令人們可以在加密的資料中進行諸如檢索、比較等操作，得出正確的結果，而在整個處理過程中無需對資料進行解密。其意義在於，真正從根本上解決將資料及其操作委託給第三方時的保密問題，可以保證實現處理者無法訪問到數據的信息。
同態加密雖具有安全性，但同態加密尚有拖慢數據運算速度的這個缺點。這個缺點將會造成影響，所以還有許多大的改善空間。
### PATE框架
